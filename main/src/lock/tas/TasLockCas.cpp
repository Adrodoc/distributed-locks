...
class TasLockCas : public Lock {
  ...
  TasLockCas(MPI_Comm comm, int master_rank, int rank)
      : TasLockCas(
            MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {}

  TasLockCas(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TasLockCas"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TasLockCas(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TasLockCas(comm, master_rank, get_rank(comm)) {}

  TasLockCas(MpiWindow window, int master_rank = 0)
      : TasLockCas(std::move(window), master_rank, get_rank(window.comm)) {}

  TasLockCas(TasLockCas &&other) = default;
