...
class TasLockBo : public Lock {
  ...
  TasLockBo(MPI_Comm comm, int master_rank, int rank)
      : TasLockBo(MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {
  }

  TasLockBo(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TasLockBo"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TasLockBo(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TasLockBo(comm, master_rank, get_rank(comm)) {}

  TasLockBo(MpiWindow window, int master_rank = 0)
      : TasLockBo(std::move(window), master_rank, get_rank(window.comm)) {}

  TasLockBo(TasLockBo &&other) = default;
