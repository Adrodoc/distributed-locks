...
class TasLock : public Lock {
  ...
  TasLock(MPI_Comm comm, int master_rank, int rank)
      : TasLock(MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {}

  TasLock(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TasLock"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TasLock(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TasLock(comm, master_rank, get_rank(comm)) {}

  TasLock(MpiWindow window, int master_rank = 0)
      : TasLock(std::move(window), master_rank, get_rank(window.comm)) {}

  TasLock(TasLock &&other) = default;
