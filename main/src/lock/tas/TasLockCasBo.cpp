...
class TasLockCasBo : public Lock {
  ...
  TasLockCasBo(MPI_Comm comm, int master_rank, int rank)
      : TasLockCasBo(
            MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {}

  TasLockCasBo(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TasLockCasBo"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TasLockCasBo(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TasLockCasBo(comm, master_rank, get_rank(comm)) {}

  TasLockCasBo(MpiWindow window, int master_rank = 0)
      : TasLockCasBo(std::move(window), master_rank, get_rank(window.comm)) {}

  TasLockCasBo(TasLockCasBo &&other) = default;
