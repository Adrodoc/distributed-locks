...
public:
    McsLockWithCohortDetection(const McsLockWithCohortDetection &) = delete;
    McsLockWithCohortDetection(const MPI_Comm comm = MPI_COMM_WORLD, const int master_rank = 0)
        : comm{comm},
          master_rank{master_rank},
          rank{get_rank(comm)}
    {
        // log() << "entering McsLockWithCohortDetection" << std::endl;
        MPI_Info info;
        ...
        MPI_Barrier(comm);
        // log() << "exiting McsLockWithCohortDetection" << std::endl;
    }
