...
class HemLockOverlapAtomic : public LockWithInlineCohortDetection {
  ...
  HemLockOverlapAtomic(MPI_Comm comm, int master_rank, int rank)
      : HemLockOverlapAtomic(
            MpiWindow::allocate_shared(get_win_size(rank == master_rank), comm), master_rank,
            rank) {}

  HemLockOverlapAtomic(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "HemLockOverlapAtomic"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  HemLockOverlapAtomic(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : HemLockOverlapAtomic(comm, master_rank, get_rank(comm)) {}

  HemLockOverlapAtomic(MpiWindow window, int master_rank = 0)
      : HemLockOverlapAtomic(std::move(window), master_rank, get_rank(window.comm)) {}
