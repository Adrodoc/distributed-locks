...
class HemLockCtrAtomic : public LockWithInlineCohortDetection {
  ...
  HemLockCtrAtomic(MPI_Comm comm, int master_rank, int rank)
      : HemLockCtrAtomic(
            MpiWindow::allocate_shared(get_win_size(rank == master_rank), comm), master_rank,
            rank) {}

  HemLockCtrAtomic(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "HemLockCtrAtomic"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  HemLockCtrAtomic(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : HemLockCtrAtomic(comm, master_rank, get_rank(comm)) {}

  HemLockCtrAtomic(MpiWindow window, int master_rank = 0)
      : HemLockCtrAtomic(std::move(window), master_rank, get_rank(window.comm)) {}
