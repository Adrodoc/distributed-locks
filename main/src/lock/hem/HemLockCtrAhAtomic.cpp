...
class HemLockCtrAhAtomic : public LockWithInlineCohortDetection {
  ...
  HemLockCtrAhAtomic(MPI_Comm comm, int master_rank, int rank)
      : HemLockCtrAhAtomic(
            MpiWindow::allocate_shared(get_win_size(rank == master_rank), comm), master_rank,
            rank) {}

  HemLockCtrAhAtomic(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "HemLockCtrAhAtomic"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  HemLockCtrAhAtomic(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : HemLockCtrAhAtomic(comm, master_rank, get_rank(comm)) {}

  HemLockCtrAhAtomic(MpiWindow window, int master_rank = 0)
      : HemLockCtrAhAtomic(std::move(window), master_rank, get_rank(window.comm)) {}
  ...
  void release() {
    ...
    if (master_mem.tail.compare_exchange_strong(
      ...
      while (mem.grant.fetch_add(0, std::memory_order_relaxed) != NULL_GRANT) {
        int flag;  // Trigger MPI progress
        MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &flag, MPI_STATUS_IGNORE);
      }
    }
