...
class TtsLockCas : public Lock {
  ...
  TtsLockCas(MPI_Comm comm, int master_rank, int rank)
      : TtsLockCas(
            MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {}

  TtsLockCas(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TtsLockCas"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TtsLockCas(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TtsLockCas(comm, master_rank, get_rank(comm)) {}

  TtsLockCas(MpiWindow window, int master_rank = 0)
      : TtsLockCas(std::move(window), master_rank, get_rank(window.comm)) {}

  TtsLockCas(TtsLockCas &&other) = default;
