...
class TtsLockAtomicWithCohortDetection : public LockWithInlineCohortDetection {
  ...
  TtsLockAtomicWithCohortDetection(MPI_Comm comm, int master_rank, int rank)
      : TtsLockAtomicWithCohortDetection(
            MpiWindow::allocate_shared(get_win_size(rank == master_rank), comm, master_rank),
            master_rank, rank) {}

  TtsLockAtomicWithCohortDetection(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TtsLockAtomicWithCohortDetection"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TtsLockAtomicWithCohortDetection(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TtsLockAtomicWithCohortDetection(comm, master_rank, get_rank(comm)) {}

  TtsLockAtomicWithCohortDetection(MpiWindow window, int master_rank = 0)
      : TtsLockAtomicWithCohortDetection(std::move(window), master_rank, get_rank(window.comm)) {}
