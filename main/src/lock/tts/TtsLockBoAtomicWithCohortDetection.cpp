...
class TtsLockBoAtomicWithCohortDetection : public LockWithInlineCohortDetection {
  static constexpr std::chrono::nanoseconds BACKOFF_MIN{1024};
  static constexpr std::chrono::nanoseconds BACKOFF_MAX{16384};
  ...
  TtsLockBoAtomicWithCohortDetection(MPI_Comm comm, int master_rank, int rank)
      : TtsLockBoAtomicWithCohortDetection(
            MpiWindow::allocate_shared(get_win_size(rank == master_rank), comm, master_rank),
            master_rank, rank) {}

  TtsLockBoAtomicWithCohortDetection(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TtsLockBoAtomicWithCohortDetection"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TtsLockBoAtomicWithCohortDetection(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TtsLockBoAtomicWithCohortDetection(comm, master_rank, get_rank(comm)) {}

  TtsLockBoAtomicWithCohortDetection(MpiWindow window, int master_rank = 0)
      : TtsLockBoAtomicWithCohortDetection(std::move(window), master_rank, get_rank(window.comm)) {}
  ...
  uint8_t acquire_cd() {
    ...
    while ((status = mem.status.exchange(WAIT, std::memory_order_acquire)) == WAIT) {
      ...
      } while (mem.status.load(std::memory_order_relaxed) == WAIT);
    }
