...
class TtsLockBo : public Lock {
  ...
  TtsLockBo(MPI_Comm comm, int master_rank, int rank)
      : TtsLockBo(MpiWindow::allocate(get_win_size(rank == master_rank), comm), master_rank, rank) {
  }

  TtsLockBo(MpiWindow window, int master_rank, int rank)
 ...
 public:
  static constexpr std::string_view NAME() { return "TtsLockBo"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) {
    return on_master_rank ? sizeof(memory_layout) : 0;
  }

  TtsLockBo(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : TtsLockBo(comm, master_rank, get_rank(comm)) {}

  TtsLockBo(MpiWindow window, int master_rank = 0)
      : TtsLockBo(std::move(window), master_rank, get_rank(window.comm)) {}

  TtsLockBo(TtsLockBo &&other) = default;
