...
class McsLockAccumulate : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockAccumulate"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  static McsLockAccumulate per_node(
  ...
  McsLockAccumulate(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockAccumulate(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockAccumulate(MpiWindow window, int master_rank = 0)
  ...
  McsLockAccumulate(McsLockAccumulate &&other) = default;
  ...
  void acquire() {
    ...
    if (predecessor != NULL_RANK) {
      ...
      MPI_Accumulate(
          &window.mem_rank, 1, MPI_INT, predecessor, next_disp, 1, MPI_INT, MPI_REPLACE,
          window.win);
  ...
  void release() {
    ...
    MPI_Accumulate(
        &FALSE, 1, MPI_CXX_BOOL, successor, locked_disp, 1, MPI_CXX_BOOL, MPI_REPLACE, window.win);
