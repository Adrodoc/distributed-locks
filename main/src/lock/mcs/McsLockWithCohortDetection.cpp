...
class McsLockWithCohortDetection : public LockWithInlineCohortDetection {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockWithCohortDetection"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  static McsLockWithCohortDetection per_node(
  ...
  McsLockWithCohortDetection(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockWithCohortDetection(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockWithCohortDetection(MpiWindow window, int master_rank = 0)
  ...
  McsLockWithCohortDetection(McsLockWithCohortDetection &&other) = default;
  ...
  void release_cd(uint8_t status) {
    ...
    MPI_Put(&status, 1, MPI_UINT8_T, successor, status_disp, 1, MPI_UINT8_T, window.win);
