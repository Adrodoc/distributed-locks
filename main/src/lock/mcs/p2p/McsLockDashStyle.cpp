...
class McsLockDashStyle : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockDashStyle"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  static McsLockDashStyle per_node(MPI_Comm global_comm, MPI_Comm local_comm, int master_rank = 0) {
    return {MpiWindow::allocate_per_node(
        get_win_size(get_rank(global_comm) == master_rank), global_comm, local_comm)};
  }

  McsLockDashStyle(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockDashStyle(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockDashStyle(MpiWindow window, int master_rank = 0)
  ...
  McsLockDashStyle(McsLockDashStyle &&other) = default;
