...
class McsLockMpiTwoSided : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockMpiTwoSided"; }

  McsLockMpiTwoSided(const MPI_Comm comm = MPI_COMM_WORLD, const int master_rank = 0)
      : McsLockMpiTwoSided(comm, master_rank, get_rank(comm), allocate_win(comm, master_rank)) {}

  McsLockMpiTwoSided(
      const MPI_Comm comm, const int master_rank, const int mem_rank, const MPI_Win win)
      : McsLockMpiTwoSided(comm, master_rank, mem_rank, (memory_layout *)get_win_base(win), win) {}
  ...
  McsLockMpiTwoSided(McsLockMpiTwoSided &&other) noexcept
      :
#ifdef STATS
  ...
  void acquire() {
    ...
    if (predecessor != NULL_RANK) {
      ...
      MPI_Put(&rank, 1, MPI_INT, predecessor, next_disp, 1, MPI_INT, win);
