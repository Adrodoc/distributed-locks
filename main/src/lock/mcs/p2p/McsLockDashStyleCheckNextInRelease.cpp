...
class McsLockDashStyleCheckNextInRelease : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockDashStyleCheckNextInRelease"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  static McsLockDashStyleCheckNextInRelease per_node(
  ...
  McsLockDashStyleCheckNextInRelease(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockDashStyleCheckNextInRelease(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockDashStyleCheckNextInRelease(MpiWindow window, int master_rank = 0)
  ...
  McsLockDashStyleCheckNextInRelease(McsLockDashStyleCheckNextInRelease &&other) = default;
  ...
  void acquire() {
    ...
    if (predecessor != NULL_RANK) {
      ...
      int result;
      MPI_Fetch_and_op(&rank, &result, MPI_INT, predecessor, next_disp, MPI_REPLACE, window.win);
      MPI_Win_flush(predecessor, window.win);

      // log() << "waiting for predecessor" << std::endl;
      MPI_Recv(NULL, 0, MPI_UINT8_T, MPI_ANY_SOURCE, 0, window.comm, MPI_STATUS_IGNORE);
    }
#ifdef STATS
    else
  ...
  void release() {
    ...
    int successor;
    MPI_Fetch_and_op(NULL, &successor, MPI_INT, window.mem_rank, next_disp, MPI_NO_OP, window.win);
    ...
    if (successor == NULL_RANK) {
      ...
      do {
        ...
        MPI_Fetch_and_op(
            NULL, &successor, MPI_INT, window.mem_rank, next_disp, MPI_NO_OP, window.win);
        MPI_Win_flush(window.mem_rank, window.win);
