...
class McsLockTwoSided : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockTwoSided"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  static McsLockTwoSided per_node(MPI_Comm global_comm, MPI_Comm local_comm, int master_rank = 0) {
    return {MpiWindow::allocate_per_node(
        get_win_size(get_rank(global_comm) == master_rank), global_comm, local_comm)};
  }

  McsLockTwoSided(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockTwoSided(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockTwoSided(MpiWindow window, int master_rank = 0)
  ...
  McsLockTwoSided(McsLockTwoSided &&other) = default;
