...
class McsLockWithTasStealing : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "McsLockWithTasStealing"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  McsLockWithTasStealing(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : McsLockWithTasStealing(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  McsLockWithTasStealing(MpiWindow window, int master_rank = 0)
  ...
  McsLockWithTasStealing(McsLockWithTasStealing &&other) = default;
