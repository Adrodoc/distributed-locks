...
class ShflLockGlobalTas : public Lock {
 ...
 public:
  static constexpr std::string_view NAME() { return "ShflLockGlobalTas"; }

  static constexpr MPI_Aint get_win_size(bool on_master_rank) { return sizeof(memory_layout); }

  ShflLockGlobalTas(MPI_Comm comm = MPI_COMM_WORLD, int master_rank = 0)
      : ShflLockGlobalTas(
            MpiWindow::allocate(get_win_size(get_rank(comm) == master_rank), comm), master_rank) {}

  ShflLockGlobalTas(MpiWindow window, int master_rank = 0)
  ...
  ShflLockGlobalTas(ShflLockGlobalTas &&other) = default;
